...
Save_to_Csv(data=train_data, file_name='DrugBank' +'_drug_cold_'+'train', Save_format='csv', Save_type='col')
test_data = {'compound_iso_smiles': test_drug, 'target_sequence': test_protein, 'affinity': test_affinity}
Save_to_Csv(data=test_data, file_name= 'DrugBank' +'_drug_cold_'+'test', Save_format='csv', Save_type='col')


# Load the dataset
data = pd.read_csv('DrugBank_drug_cold_train.csv')
...
for state in random_states:
    ...
    for fold_number, (train_index, test_index) in enumerate(kf.split(data), start=1):
        ...
        fold_dir = os.path.join('D',str(state), f'fold_{fold_number}')
