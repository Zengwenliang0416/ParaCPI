...
for i in range(len(drug)):
    if (c[i] in did_sample) and (p[i] in pid_sample):
        test_drug.append(drug[i])
        test_protein.append(protein[i])
        test_affinity.append(affinity[i])
        test_pid.append(p[i])
        test_did.append(c[i])
    elif (c[i] not in did_sample) and (p[i] not in pid_sample):
...
Save_to_Csv(data=train_data, file_name='DrugBank' +'_drug_protein_cold_'+'train', Save_format='csv', Save_type='col')
test_data = {'compound_iso_smiles': test_drug, 'target_sequence': test_protein, 'affinity': test_affinity}
Save_to_Csv(data=test_data, file_name= 'DrugBank' +'_drug_protein_cold_'+'test', Save_format='csv', Save_type='col')

# Load the dataset
data = pd.read_csv('DrugBank_drug_protein_cold_train.csv')
...
for state in random_states:
    ...
    for fold_number, (train_index, test_index) in enumerate(kf.split(data), start=1):
        ...
        fold_dir = os.path.join("DP",str(state), f'fold_{fold_number}')
